...
describe('User routes', () => {
  ...
  describe('PATCH /user/:userId', () => {
    test('should return 200 and successfully update user if data is ok as admin', async () => {
      await insertUsers([userOne]);
      const updateBody = {
        name: faker.name.findName(),
        email: faker.internet.email().toLowerCase(),
      ...
      expect(dbUser).toMatchObject({ name: updateBody.name, email: updateBody.email, role: 'admin' });
    });
